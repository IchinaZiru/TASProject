...
function Inventory()
{
	...
	Convars.SetValue("host_timescale", 0.25);	//scale of time
	
	local hPlayer = null;
	if ((hPlayer = Ent("!nick")) != null)
	{
		SetClientName(hPlayer, "null");
		hPlayer.GiveItem("pistol");hPlayer.GiveItem("vomitjar");
		hPlayer.SetHealth(50);
		TeleportEntity(hPlayer,  Vector(6278.771, 8397.507, 0.031), Vector(9.359, -136.979, 0.000));
		ST_MR(Ent("!nick"), true, "m3nick");
		/*
			============== ST_MR usage ==============
	...
	{
		hPlayer.GiveItem("pistol");hPlayer.GiveItem("pipe_bomb");
		hPlayer.SetHealth(50);
		TeleportEntity(hPlayer,  Vector(6284.908, 8341.400, 0.031), Vector(8.740, 174.468, 0.000));
		//ST_MR(hPlayer, 0, "ellis");
	}
}
